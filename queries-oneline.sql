...
SELECT DISTINCT departamento.nombre FROM universidad.grado INNER JOIN asignatura ON asignatura.id_grado = grado.id LEFT JOIN profesor ON profesor.id_profesor = asignatura.id_profesor LEFT JOIN departamento ON departamento.id = profesor.id_departamento WHERE NOT asignatura.id_profesor IS NULL;
...
SELECT DISTINCT departamento.* FROM departamento LEFT JOIN profesor ON profesor.id_departamento = departamento.id LEFT JOIN asignatura ON asignatura.id_profesor = profesor.id_profesor LEFT JOIN curso_escolar ON curso_escolar.id = asignatura.curso WHERE asignatura.nombre IS NULL AND asignatura.curso IS NULL;
/* CONSULTES RESUM */
